...
class PNCC():
    ...
    # Low-pass filter for ECG signal below 20 Hz
    def low_pass_filter(self, signal, cutoff, fs, order=5):
        nyq = 0.5 * fs
        normal_cutoff = cutoff / nyq
        b, a = butter(order, normal_cutoff, btype='low', analog=False)
        return filtfilt(b, a, signal)

    # High-pass and low-pass filters for PCG signal
    def high_low_pass_filter(self, signal, lowcut, highcut, fs, order=5):
        nyq = 0.5 * fs
        low = lowcut / nyq
        high = highcut / nyq
    ...
    def extract_ecg_features(self, dat_file, fs=1000):
        ...
        # Low-pass filter (below 20 Hz)
        ecg_filtered = self.low_pass_filter(ecg_filtered, 20, fs)
    ...
    # Extract PCG features using wavelet transform and Hilbert transform
    def extract_pcg_features(self, wav_file, sr=1000):
        ...
        filtered_pcg = self.high_low_pass_filter(wav_data, 25, 400, sr)

        # Wavelet transform for multi-scale feature extraction
        coeffs = pywt.wavedec(filtered_pcg, 'db4', level=5)
        cA5 = coeffs[0]
    ...
    def data_process(self):
        ...
        for idx, (wav_file, dat_file) in enumerate(zip(wav_files, dat_files)):
            ...
            if not record_info.empty and record_info['reliability'].values[0] == 1:
                ...
                ecg_mel.append(np.resize(ecg_mel_spectrogram, (224, 224)))
                pcg_mel.append(np.resize(pcg_mel_spectrogram, (224, 224)))
